package io.github.packagewjx.brandreportbackend.domain;

public class Constants {
    /**
     * 年度数据
     */
    public static final String PERIOD_ANNUAL = "annual";
    /**
     * 月度数据
     */
    public static final String PERIOD_MONTHLY = "monthly";
    /**
     * 季度数据
     */
    public static final String PERIOD_QUARTERLY = "quarterly";

    /**
     * 默认统计周期，与数据集和报告的统计周期一致。仅能用于Index中
     */
    public static final String PERIOD_DEFAULT = "default";

    /**
     * 用于注解中表示true
     */
    public static final String ANNOTATION_VALUE_TRUE = "true";

    /**
     * 用于注解中表示false
     */
    public static final String ANNOTATION_VALUE_FALSE = "false";
}
